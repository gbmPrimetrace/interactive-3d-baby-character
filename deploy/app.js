class InteractiveBabyCharacter {
    ...
    async setupHDRI() {
        return new Promise((resolve, reject) => {
            ...
            console.log('Loading HDRI from: src/hdri.exr');
            exrLoader.load(
                'src/hdri.exr',
                (hdriTexture) => {
                    try {
    ...
    setupControls() {
        ...
        // Set initial target to focus on the model
    ...
    setupSpeechRecognition() {
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            this.recognition.grammars = null; // Disable grammar for better mobile compatibility
    ...
    async loadBabyModel() {
        ...
        try {
            ...
            this.updateStatus('Error loading 3D model');
        }
    }
        ...
        this.babyModel.traverse((child) => {
            if (child.isMesh) {
                ...
                if (isBodyMesh) {
                    ...
                    const alphaMap = textureLoader.load('src/body__Opacity.jpg');
